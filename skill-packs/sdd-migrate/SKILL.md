...
metadata:
  ...
  version: "1.0"
---
## sdd-migrate — legacy non-Git SDD → unified model (one-off, advanced)
...
3. **Lifecycle.** Status is unchanged; from now on `sdd-spec-review` drives transitions over Git (no `_vN`).
...
### After
`git log` shows the migration commits; specs validate against `sdd-spec-schema`; run `sdd-audit-*` before sign-off.
