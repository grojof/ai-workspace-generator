...
metadata:
  ...
  version: "1.0"
---
## sdd-spec-review — revise a spec & drive the status lifecycle (REASONS mode)

The maintenance counterpart of `sdd-spec-capture`: every change to a saved spec, plus the **status
...
### Edits
Classify: content edit · structural add (reuse `sdd-spec-capture`'s per-section questions) · removal
(cascade-check §2↔§5 references) · flag toggle (include/exclude §3.3 auth, §3.4 storage, §7.2 PD) · profile
promotion B→A (structural rewrite, re-run `sdd-onboarding`). Re-validate against `sdd-spec-schema` before
...
| From → To | Gate | Effect |
|---|---|---|
| draft → user-reviewed | full `sdd-spec-schema` checklist green; `reviewers` non-empty; UI specs: mockup accepted | §0 sign-off bullet (`Reviewer: <email>`); **no version bump** |
