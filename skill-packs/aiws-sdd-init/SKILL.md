...
metadata:
  ...
  version: "1.0"
---
## aiws-sdd-init — scaffold the project layout (REASONS mode)
...
### Pre-flight
- Read the profile (A | B) and capability flags from the `aiws-sdd-onboarding` decision (or ask to run it first).
...
`_vN` suffixes). Record the scaffold decision in the change-folder. Hand off to `aiws-sdd-spec-capture`.
