...
metadata:
  ...
  version: "1.0"
---
## sdd-code-generation — translate a spec into code (REASONS mode)
...
- **Refuse `draft`/`retired`**; accept `user-reviewed`/`it-approved`. Re-validate against `sdd-spec-schema`.
...
Record the run (files + SHA-256 + TODOs) in `{{paths.changes}}/<change>/`. Hand off to `sdd-test-generation`.
