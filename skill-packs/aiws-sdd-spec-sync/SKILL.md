...
metadata:
  ...
  version: "1.0"
---
## aiws-sdd-spec-sync — keep the REASONS Canvas in sync with the code (SPDD)
...
> Canvas, then propagate with `aiws-sdd-code-maintenance`. Use this skill only for code-side changes that do
> **not** change intent.
...
2. **Classify** each diff: *non-behavioural* (refactor/rename/extract → sync into the Canvas) vs
   *behavioural* (**STOP** — that is a logic change; route to `aiws-sdd-code-maintenance`, prompt-first).
...
- A behavioural change is **out of scope** here — it must update the prompt first (`aiws-sdd-code-maintenance`).
