...
[docs/decisions/0001-mixed-sdd.md](../../docs/decisions/0001-mixed-sdd.md) and
[docs/SDD-UPSTREAM.md](../../docs/SDD-UPSTREAM.md).

## Do this

1. **Read the tracker.** Open [docs/SDD-UPSTREAM.md](../../docs/SDD-UPSTREAM.md). Note the two upstreams,
...
3. **Filter to philosophy only.** Keep changes to the *workflow/concepts*: a new lifecycle stage, a
   ...
   packaging, `.specify/` / `/speckit.*` / `/opsx:*` tooling — by [ADR 0001](../../docs/decisions/0001-mixed-sdd.md)
...
4. **Propose, don't auto-apply.** For each relevant change, draft the concrete edit to our concept
   ...
   Update the provenance table and **Last reviewed** dates in both
   [docs/SDD-UPSTREAM.md](../../docs/SDD-UPSTREAM.md) and [docs/es/SDD-UPSTREAM.md](../../docs/es/SDD-UPSTREAM.md).
...
6. **If you did change generated output:** bump `TEMPLATES_VERSION` in [`src/version.ts`](../../src/version.ts),
   run `npm run build`, and verify the idempotency invariants in [docs/MAINTAINING.md](../../docs/MAINTAINING.md).
