...
More feature blocks: `sdd` (if `sdd.enabled`), `living-docs` (if `livingDocs`), and `imported` (added by
`ai-workspace import`).
...
tool's **native** skills (lean SDD flow, living-docs, guides) are generated by code.
