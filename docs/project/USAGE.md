...
> Since late 2025 Copilot reads **`AGENTS.md`** natively (root, primary), so the generated
> `.github/copilot-instructions.md` is a **thin pointer** to it — not a full mirror — to avoid loading the same
> content twice. The repo-wide rules live in `AGENTS.md`; only the path-scoped `instructions/*` and `prompts/*`
> (which `AGENTS.md` does not cover) carry their own content.
