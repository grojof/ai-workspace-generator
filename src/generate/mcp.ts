...
export function buildCodexMcp(ids: string[]): string {
  const servers = serversFor(ids);
  const lines = ["# Codex MCP servers — generated by ai-workspace. See https://developers.openai.com/codex/mcp"];
  for (const [id, def] of Object.entries(servers)) {
    lines.push("", `[mcp_servers.${id}]`, `command = ${tomlStr(def.command)}`, `args = [${def.args.map(tomlStr).join(", ")}]`);
    if (def.env) {
