...
export async function runInit(cwd: string, options: InitOptions = {}): Promise<void> {
  ...
  if (options.advanced) mode = "advanced";
      ...
      options: [
        {
          ...
          hint: "pocas preguntas + defaults; acepta el stack detectado",
        },
    ...
    note(
      "For the richest setup, run the aiws-configure-workspace skill (/aiws-configure) and let the AI propose your config.",
  ...
  if (mode === "advanced") {
    ...
    const projectMode = await select({
      ...
      options: [
        { value: "new", label: "New / Nuevo", hint: "usa versiones estables actuales" },
        {
          ...
          hint: "conserva versiones; sube solo con evaluación",
        },
      ],
    ...
    note(
      "Detection seeds the stack only — your user type & experience are your choice (they set the governance posture).",
      "Profile / Perfil",
    );
    const userType = await select({
      ...
      options: [
        {
          value: "technical",
          label: "Technical / Técnico",
          hint: "desarrollo, devops, datos, sistemas, infra",
        },
        {
          value: "business",
          label: "Business / Negocio",
          hint: "procesos, documentación, análisis, gestión",
        },
      ],
    ...
    const experience = await select({
      ...
      options: [
        {
          value: "beginner",
          label: "Beginner / Aprendiz",
          hint: "guía clara, pocas decisiones, caminos seguros",
        },
    ...
    const company = await select({
      ...
      options: [
        ...
        {
          value: "example",
          label: "Example Co (plantilla)",
          hint: "overlay de empresa de ejemplo — renómbralo/extiéndelo",
        },
      ],
      initialValue: "none",
    });
    ...
    const targets = await multiselect({
      ...
      options: [
        ...
        { value: "codex", label: "OpenAI Codex", hint: "AGENTS.md + .codex/config.toml" },
        {
          value: "opencode",
          label: "OpenCode",
          hint: "AGENTS.md + .claude/skills nativos; .opencode/opencode.json (MCP)",
        },
      ],
    ...
    const langDefaults = detected.languages.length ? detected.languages.map((l) => l.id) : ["typescript"];
    ...
    const customize = await confirm({
      message:
        "Customize advanced options? / ¿Ajustar opciones avanzadas? (purpose · SDD storage · living docs · context7)",
      initialValue: false,
    });
    bail(customize);
    ...
    if (customize) {
      ...
      if (sddEnabled) {
        ...
        const backend = await select({
          ...
          options: [
            {
              value: "files",
              label: "Files in repo (OpenSpec layout)",
              hint: "recomendado — portable, versionado en git, sin dependencias",
            },
            {
              value: "hybrid",
              label: "Files + cross-session memory (engram)",
              hint: "añade memoria entre sesiones si está disponible; los ficheros siguen siendo canónicos",
            },
        ...
        if (experience === "advanced") {
          const method = await select({
            ...
            options: [
              {
                value: "sdd",
                label: "Spec-Driven (SDD)",
                hint: "recomendado — spec → diseño → tareas → código",
              },
              {
                value: "spdd",
                label: "Structured-Prompt-Driven (SPDD)",
                hint: "el prompt (REASONS Canvas) es un artefacto versionado; implica schema reasons",
              },
            ],
            initialValue: "sdd",
          });
      ...
      const vsc = await confirm({
        message:
          "Generate .vscode/ recommendations (extensions/settings/mcp)? Turn off for Visual Studio / non-VS-Code.",
        initialValue: true,
      });
      bail(vsc);
      vscode = vsc === true;
    ...
    inputs = {
      ...
      targets: targets as ("claude" | "copilot" | "codex" | "opencode")[],
      vscode,
    ...
    inputs = simpleDefaults(detected, {
      ...
      targets: sTargets as ("claude" | "copilot" | "codex" | "opencode")[],
