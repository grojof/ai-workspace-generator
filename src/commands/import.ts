...
import { docsPaths } from "../generate/paths.js";
...
export function runImport(cwd: string, sources: string[]): void {
  ...
  const reconcilePath = `${status}/INGEST-RECONCILE.md`;
  ...
  for (const a of [...byKind("instructions"), ...byKind("company")]) {
    const dest = resolve(cwd, `${status}/imported`, basename(a.rel));
    const r = writeIfMissing(dest, readFileSync(a.abs, "utf8"));
    if (r.status !== "unchanged") copied.push(`${status}/imported/${basename(a.rel)}`);
  }
  ...
  const refLines = [
    ...
    ...copied.map((c) => `- \`${c}\``),
    "",
    `> See \`${reconcilePath}\` for the reconciliation checklist.`,
  ].join("\n");
  ...
  writeFile(resolve(cwd, reconcilePath), checklist);
