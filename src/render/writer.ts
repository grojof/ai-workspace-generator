...
/**
 * Copy a binary file byte-for-byte (idempotent) — for skill assets (templates, logos) that must NOT be
 * utf8-normalized or get a trailing newline. Compares bytes so a re-run reports `unchanged`.
 */
export function writeBinary(absPath: string, srcPath: string): WriteResult {
  const next = readFileSync(srcPath);
  const before = existsSync(absPath) ? readFileSync(absPath) : null;
  if (before && before.equals(next)) {
    return { path: absPath, status: "unchanged" };
  }
  if (dryRun) {
    planned.set(absPath, "<binary>");
    return { path: absPath, status: before ? "updated" : "created" };
  }
  ensureDir(absPath);
